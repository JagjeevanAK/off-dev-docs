...
- **FoodVisor Contributed (Python/MongoDB) backend**: The FoodVisor startup contributed a few years ago [a Python-based backend with a MongoDB export](https://github.com/openfoodfacts/openfoodfacts-apirestpython), providing a solid starting point for caching in Python environments.
...
- **SDKs**: We have [a number of official SDKs](../index.mdx#sdks) that can be leveraged as part of a caching backend. Please leverage and contribute to those 🙏
...
For more on legal issues [please read this page](./license-be-on-the-legal-side.mdx)
...
Currently, there's no public API for real-time OFF updates stream. However, we have an internal system (REDIS) and are exploring options for a future 3rd party notification API. Express your interest by contacting [reuse@openfoodfacts.org](mailto:reuse@openfoodfacts.org).
