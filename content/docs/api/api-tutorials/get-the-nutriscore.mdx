...
- Please refer to the [product addition tutorial](../tutorials/adding-missing-products.mdx) for the technical way to do the required operations (such as nutrition input), and to the high level workflow below for all the cases you have to handle.
...
- Replace like so `nutriscore-e-new-en.svg` > `nutriscore-e-new-fr.svg`

You can get the new assets in additional languages by contacting [reuse@openfoodfacts.org](mailto:reuse@openfoodfacts.org). We will provide unofficial courtesy translations.
...
- You can get states with https://world.openfoodfacts.org/api/v0/product/3414280980209.json?fields=ecoscore_grade,states_tags
...
{/* Note: We now have a convenient API to automatically extract nutrition from an image */}
